import type { MenuItem } from '@/types/teacherDashboard';
...
export function Sidebar({ menuItems }: SidebarProps) {
  return (
    <div className="w-64 bg-white shadow-sm border-l">
      ...
      <div className="p-4">
        <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-4">منوی اصلی</div>
        <nav className="space-y-2">
          {menuItems.map((item, index) => (
            ...
            >
              ...
              {item.badge && (
                <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded-full">{item.badge}</span>
              )}
            </a>
          ))}
        </nav>
